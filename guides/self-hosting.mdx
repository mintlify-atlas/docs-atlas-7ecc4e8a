---
title: "Self-Host Discord Rawrbot on Your Machine"
...
Self-hosting Rawrbot lets you run your own instance with your own bot token, giving you full control over the bot and its configuration. You'll need Python 3.10 or newer, a Discord bot application created through the Discord Developer Portal, and optionally Spotify API credentials if you want to use the Spotify-related commands.

## Prerequisites

Before you start, make sure you have the following:

- **Python 3.10 or newer** — the codebase uses f-strings and type hint syntax that require at least Python 3.10. Some expressions in `cogs/moderation.py` (such as `f"{"" if ... else "s"}"`) use nested f-string syntax that requires Python 3.12 or newer.
...
- **`yt-dlp` and `ffprobe`** — only required if you intend to use the `/upload` command. These are not listed in `requirements.txt` and must be installed separately on your system.
...
<Warning>
  The owner user ID (`717471432816459840`) is hardcoded in both `cogs/general.py` (in `bigDict["sync"]["extras"]["allowedUsers"]`) and `cogs/moderation.py` (in the `/timeout` command handler). If you are self-hosting, you **must** replace these IDs with your own Discord user ID before running the bot, otherwise you will not be able to use owner-only commands.
</Warning>
