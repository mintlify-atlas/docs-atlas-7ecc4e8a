---
title: "Bot Permissions and Command Access Control"
...
<Warning>
  The owner user ID (`717471432816459840`) is hardcoded in `cogs/general.py` and `cogs/moderation.py`. If you are self-hosting Rawrbot, you must update these values to your own Discord user ID before running the bot. See the [Self-Hosting](/guides/self-hosting) guide for instructions on finding your user ID.
</Warning>
...
Note that `/sync` is unique: it is available in DMs but restricted to user install only (`allowed_installs(False, True)`). This means it can only be invoked by a user who has installed the bot to their account — it is intentionally not exposed as a guild command to avoid accidental syncs from server members.
